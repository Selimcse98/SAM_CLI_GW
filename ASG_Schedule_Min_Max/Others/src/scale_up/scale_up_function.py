...
def lambda_handler(event, context):
    """
    This function scales up an Auto Scaling Group by setting its desired and minimum capacities to its maximum capacity.
    """
    ...
    try:
        ...
        if not response['AutoScalingGroups']:
            ...
            send_sns_notification(sns_topic_arn, f"Scale-Up Failed for {asg_name}", message)
        ...
        message = (
            f"Successfully scaled up the Auto Scaling Group '{asg_name}'.\n\n"
            f"Previous State:\n"
            f"- Desired Capacity: {current_desired}\n"
            f"- Min Size: {current_min}\n\n"
            f"New State (scaled to Max Size):\n"
            f"- Desired Capacity: {max_size}\n"
            f"- Min Size: {max_size}"
        )
        print(message)
        send_sns_notification(sns_topic_arn, f"Scale-Up Successful for {asg_name}", message)
        ...
        print(error_message)
        send_sns_notification(sns_topic_arn, f"Scale-Up Failed for {asg_name}", error_message)
